...
const fetchPokemon = async function(searchValue) {
    try {
        ...
        renderError();
        console.log("Pokemon not Found!", err1);
    }
};
...
const updateSuggestions = async function(searchValue) {
    try {
        if (searchValue !== "") {
            ...
            const data = await response.json();
            const suggestions = data.results// don't return suggestion if searchInput === pokemon name
            .filter((pokemon)=>{
                return pokemon.name.startsWith(searchValue) && pokemon.name.toLowerCase() !== searchValue.toLowerCase();
            }).map((pokemon)=>pokemon.name);
            const datalist = getElement("#search-suggestions");
            datalist.innerHTML = suggestions.map((suggestion)=>`<option value="${suggestion}"></option>`).slice(0, 9).join("");
        }
    } catch (err1) {
        console.log("Error retrieving search suggestions!", err1);
    }
};
...
const renderHtml = function() {
    try {
        ...
        const abilities = abilitiesArray.slice(0, 2).map((ability)=>ability.ability.name).join(", ");
...
searchInput.addEventListener("keydown", (e)=>{
    if (e.keyCode === 13) loadHelper(e, getPokemon);
});
...
},{}]},["lKzq4","bNKaB"], "bNKaB", "parcelRequiref32a")
